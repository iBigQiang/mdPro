...
## 2026-01-04 00:28 (v1.2.23) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 23:59 (v1.2.22) 自动更新

- 修复（让标题跟页眉右侧对齐）

## 2026-01-03 23:48 (v1.2.21) 自动更新

- 已修复导出 PDF 的页眉页脚标题显示/对齐问题

## 2026-01-03 23:10 (v1.2.20) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 23:03 (v1.2.19) 自动更新

- release 脚本已再次增强，现在完美支持你的工作流：

## 2026-01-03 22:56 (v1.2.18) 自动更新

- “向上修正位置：将标题位置由 -10mm 进一步上移至 -14mm，这应该能让它与左侧的系统时间处于完全水平的位置。 ”

## 2026-01-03 22:26 (v1.2.17) 自动更新

调整 PDF 标题定位：

- 修正定位：不再使用负值（可能导致被浏览器判定为溢出而跑到页面底部），改用 top: 0 紧贴内容区顶部，同时给正文增加了 padding-top 防止重叠。现在标题应该稳稳地显示在右上角了。
- 关于页码：要显示“页码”，请务必勾选打印设置里的“页眉和页脚”。因为浏览器限制，如果不勾选此项，页码也会一同消失。不必担心“about:blank”，我已经把它彻底隐藏了。

## 2026-01-03 22:26 (v1.2.16) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 22:25 (v1.2.15) 自动更新

- 移除 "about:blank"：我将 Title 设置为了一个特殊的隐形字符（Braille Blank），这能完美欺骗浏览器，既不显示任何文字，也不会回退显示 "about:blank"。
- 标题归位：重新实现了标题的右上角对齐。
- 效果：现在导出 PDF，确保勾选“页眉和页脚”，你应该会看到：
  - 页眉：左上[时间] ———— 右上[标题] (中间空白)
  - 页脚：左下[网址] ———— 右下[页码]

## 2026-01-03 22:07 (v1.2.14) 自动更新

- 测试自动提交工作流

## 2026-01-03 21:50 (v1.2.13) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 21:28 (v1.2.12) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 20:59 (v1.2.11) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 20:39 (v1.2.10) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 20:29 (v1.2.9) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 20:26 (v1.2.8) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 20:13 (v1.2.7) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 20:01 (v1.2.6) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 19:52 (v1.2.5) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 15:49 (v1.2.4) 自动更新

- (无代码变动或仅有 release 提交)

## 2026-01-03 15:00 (v1.2.3) 微信公众号 h2 标题样式兼容性优化

- 目标：解决复制内容到微信公众号编辑器后，h2 标题的渐变底边效果失效的问题。
- 问题原因：
  - 微信公众号编辑器会过滤 CSS 变量（`var()`）和 `calc()` 函数。
  - 原 CSS 中使用了 `var(--brand-green)` 和 `calc(100% - 15px)` 等语法，导致在公众号环境中失效。
- 关键改动：
  - `WXGreen.css`: 将 h2 标题样式中的 CSS 变量替换为硬编码的颜色值 `#009874`。
  - `WXGreen.css`: 将 `linear-gradient` 中的 `calc()` 表达式替换为固定的百分比值（`60%`、`65%`）。

## 2025-12-23 14:20 (v1.2.2) 表格样式优化与资源本地化

- 目标：优化阅读体验并提升资源加载稳定性。
- 关键改动：
  - **表格水平居中**：优化了 `WXColour.css` 和 `WXGreen.css` 中的表格样式，将 `margin` 调整为 `1em auto`，确保表格在不同屏幕尺寸下均能水平居中显示。
  - **资源本地化**：将首页默认填充内容中的二维码图片替换为本地资源，避免第三方图床不稳定的问题。

## 2025-12-23 13:50 (v1.2.1) “多彩微信”主题超链接样式优化

- 目标：确保文档中的超链接颜色与当前选择的主题色保持一致，提升阅读体验的一致性。
- 关键改动：
  - **超链接颜色联动**：`WXColour.css` 中的超链接（`a` 标签）现在使用动态变量 `var(--brand-color)`，不再固定为蓝色。当切换主题色（如活力橘、玫瑰金）时，文中的链接颜色也会同步变化。

## 2025-12-23 13:25 (v1.2.0) 移动端适配升级与体验闭环

- 目标：彻底打通“多彩微信”主题的使用链路，并专项优化移动端 UI。
- 关键改动：
  - **移动端页眉精简**：在手机访问时，隐藏无意义的“手机/电脑/全屏”预览模式切换，仅保留核心的“复制”、“导出”和“样式”按钮，页眉更清爽。
  - **应用初始状态优化**：
    - 默认主题切换为“多彩微信”。
    - 默认主题色设为“翡翠绿”。
    - “样式”面板默认展开，引导用户第一时间体验动态配色。
  - **样式细节修复**：修复了 `WXColour.css` 中相邻标题间的渲染残留问题。

## 2025-12-23 13:17 (v1.1.9) 优化初始体验与样式细节

- 目标：提升新用户首次使用的“第一印象”，并修复主题样式小瑕疵。
- 关键改动：
  - **默认状态设置**：将“多彩微信”主题设为应用开启后的默认选中项，且“样式”面板默认展开，方便用户快速调节。
  - **样式修复**：修复了 `WXColour.css` 中，当 H2 标题后直接跟随 H3 标题时，因冗余伪元素导致的绿色方块样式残留问题。

## 2025-12-23 13:05 (v1.1.8) 优化“多彩微信”下划线颜色联动

- 目标：确保下划线（`++text++`）颜色能跟随主题色变化，并保持优雅的渐变效果。
- 关键改动：
  - `WXColour.css`: 重构 `.markup-underline` 样式，移除硬编码颜色，改用 `linear-gradient` 配合 `color-mix`，自动派生于 `var(--brand-color)`。

## 2025-12-23 12:53 (v1.1.7) 全新“多彩微信”主题上线

- 目标：推出一款更现代、更年轻化的主题，支持动态换色。
- 关键改动：
  - 新增主题 `WXColour.css`。
  - 支持 8 种预设主题色切换。
